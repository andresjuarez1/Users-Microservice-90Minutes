...
export class UserNotificationSaga {
    ...
    async sendNotificationNewUser(email: string, token: string, name: string, address: string, lastName: string, uuid: string, discount: number): Promise<void> {
        ...
        try {
            ...
            channel.publish(this.exchangeName, this.routingKey, Buffer.from(JSON.stringify({ "email": email, "token": token, "name": name, "string": address, "uuid": uuid, "lastName": lastName, "discount": discount})));
